...
enum FrustumSide
{
	RIGH	= 0,		// The RIGHT side of the frustum
	LEF	= 1,		// The LEFT	 side of the frustum
	BOTTO	= 2,		// The BOTTOM side of the frustum
	TO		= 3,		// The TOP side of the frustum
	BAC	= 4,		// The BACK	side of the frustum
	FRON	= 5			// The FRONT side of the frustum
}; 
...
{    
	...
	m_Frustum[RIGH][A] = clip[ 3] - clip[ 0];
	m_Frustum[RIGH][B] = clip[ 7] - clip[ 4];
	m_Frustum[RIGH][C] = clip[11] - clip[ 8];
	m_Frustum[RIGH][D] = clip[15] - clip[12];

	// Now that we have a normal (A,B,C) and a distance (D) to the plane,
	// we want to normalize that normal and distance.

	// Normalize the RIGHT side
	NormalizePlane(m_Frustum, RIGH);

	// This will extract the LEFT side of the frustum
	m_Frustum[LEF][A] = clip[ 3] + clip[ 0];
	m_Frustum[LEF][B] = clip[ 7] + clip[ 4];
	m_Frustum[LEF][C] = clip[11] + clip[ 8];
	m_Frustum[LEF][D] = clip[15] + clip[12];

	// Normalize the LEFT side
	NormalizePlane(m_Frustum, LEF);

	// This will extract the BOTTOM side of the frustum
	m_Frustum[BOTTO][A] = clip[ 3] + clip[ 1];
	m_Frustum[BOTTO][B] = clip[ 7] + clip[ 5];
	m_Frustum[BOTTO][C] = clip[11] + clip[ 9];
	m_Frustum[BOTTO][D] = clip[15] + clip[13];

	// Normalize the BOTTOM side
	NormalizePlane(m_Frustum, BOTTO);

	// This will extract the TOP side of the frustum
	m_Frustum[TO][A] = clip[ 3] - clip[ 1];
	m_Frustum[TO][B] = clip[ 7] - clip[ 5];
	m_Frustum[TO][C] = clip[11] - clip[ 9];
	m_Frustum[TO][D] = clip[15] - clip[13];

	// Normalize the TOP side
	NormalizePlane(m_Frustum, TO);

	// This will extract the BACK side of the frustum
	m_Frustum[BAC][A] = clip[ 3] - clip[ 2];
	m_Frustum[BAC][B] = clip[ 7] - clip[ 6];
	m_Frustum[BAC][C] = clip[11] - clip[10];
	m_Frustum[BAC][D] = clip[15] - clip[14];

	// Normalize the BACK side
	NormalizePlane(m_Frustum, BAC);

	// This will extract the FRONT side of the frustum
	m_Frustum[FRON][A] = clip[ 3] + clip[ 2];
	m_Frustum[FRON][B] = clip[ 7] + clip[ 6];
	m_Frustum[FRON][C] = clip[11] + clip[10];
	m_Frustum[FRON][D] = clip[15] + clip[14];

	// Normalize the FRONT side
	NormalizePlane(m_Frustum, FRON);
}
...
{
	if(PointInFrustum(Minimo.x,Minimo.y,Minimo.z)) return true;
	if(PointInFrustum(Maximo.x,Minimo.y,Minimo.z)) return true;
	if(PointInFrustum(Minimo.x,Maximo.y,Minimo.z)) return true;
	if(PointInFrustum(Maximo.x,Maximo.y,Minimo.z)) return true;
	if(PointInFrustum(Minimo.x,Minimo.y,Maximo.z)) return true;
	if(PointInFrustum(Maximo.x,Minimo.y,Maximo.z)) return true;
	if(PointInFrustum(Minimo.x,Maximo.y,Maximo.z)) return true;
	if(PointInFrustum(Maximo.x,Maximo.y,Maximo.z)) return true;
	return false;
}
